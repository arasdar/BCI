...
%                                     'circles' small circles where electrodes are located
%                                        {default = 'both'}
...
%                'circleradius'     - radius of the small circles if 'style' == 'circles'
%
...
if ~strcmp(STYLE,'blank')
  ...
  elseif strcmp(STYLE, 'circles') 
      for garett=1:length(x)
          [gX,gY,gZ]=sphere(40);         
          gX = CIRCLERADIUS*gX + x(garett);
          gY = CIRCLERADIUS*gY + y(garett);
          gZ = 0*gZ - 0;
          hold on;
          surf(gY,gX,gZ,repmat(Vl(garett),size(gX)-1),'EdgeColor', 'none');
      end      
  else
    error('Invalid style')
  end
